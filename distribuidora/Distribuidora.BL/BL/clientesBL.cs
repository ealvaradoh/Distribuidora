...
namespace Distribuidora.BL.BL
{
    public class clientesBL
    {
        ...
        public void GuardarCliente(cliente clienteGuardado)
        {
            ...
            using (_contexto = contexto.crearConexion())
            {
                string sql = "INSERT INTO cliente VALUES(@clien_id, @clien_numid, @clien_nom, @clien_tel);";
        ...
        public void EditarCliente(cliente clienteEditado)
        {
            ...
            using (_contexto = contexto.crearConexion())
            {
                string sql = "UPDATE cliente SET clien_numid=@clien_numid, clien_nom=@clien_nom, clien_tel=@clien_tel WHERE clien_id = @clien_id;";
                using (MySqlCommand comando = new MySqlCommand(sql, _contexto))
                {
        ...
        public void EliminarCliente(cliente clienteEliminado)
        {
            ...
            using (_contexto = contexto.crearConexion())
            {
                string sql = "DELETE FROM cliente WHERE clien_id = @clien_id;";
                using (MySqlCommand comando = new MySqlCommand(sql, _contexto))
                {
